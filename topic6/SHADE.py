...
class Individual:
    ...
    def random_initial(self, n):
        random_num = np.array([SHADE.get_random_num() for i in range(n)])
    ...
    def calc_fitness(self):
        ...
        这里是原始的函数y值，不能直接使用
        """
        self.fitness = SHADE.test_func(self.genes)
...
class Population:
    ...
    def calc_all_fitness(self, is_minimize=False):
        ...
        if is_minimize:  # 最小值问题，适应值取倒数，原值越小适应值越大+log1p
            old_gen = copy.deepcopy(self.individuals)
            self.clear()
            for i in range(n_individual):
                new_value = np.log(round(1 / old_gen[i].fitness, 5))
...
class SHADE:
    ...
    适应度函数：计算每个个体的适应性。
    选择，交叉，变异
    超参数的设定十分重要，突变的步伐迈得过大，将永远无法收敛的最优值->自适应
    """
    value_range = None

    def __init__(self, **params):
        self.n = params.get('dimension')  # 基因维度
        self.pool_size = params.get('pool_size')
        self.n_individual = params.get('n_individual')
        self.k = 0  # record the pool's pos
        self.params = params
        self.population = Population()
        self.temp_popu = Population()
        self.mu_cr = self.mu_f = None
        ...
        SHADE.value_range = params.get('value_range')
    ...
    def generate_random_nums(self, dist):
        random_choose = np.random.randint(0, self.pool_size, self.n_individual)
        random_nums = []
        if dist == "norm":
            random_nums = [np.random.normal(self.mu_cr[r], 0.1) for r in random_choose]
            random_nums = np.clip(random_nums, 0, 1)

        elif dist == "cauchy":
            for r in random_choose:
                random_num = stats.cauchy.rvs(self.mu_f[r], 0.1)
                while random_num < 0 or random_num > 1:
                    random_num = stats.cauchy.rvs(self.mu_f[r], 0.1)
    ...
    def initial(self, n_individual: int):
        ...
        self.mu_cr = self.mu_f = np.array([0.5 for i in range(self.pool_size)])
        self.sf = np.array([])
        self.k = 0
        self.scr = np.array([])
        self.delta_f = np.array([])
        self.crossover_rate = self.generate_random_nums(dist='norm')
        self.scaling_factor = self.generate_random_nums(dist='cauchy')  # 第一步，初始化cr和f
        for i in range(n_individual):
    ...
    def selection(self):
        ...
        if len(self.sf) > 0:  # 不为空集，更新选择池
            w_k = self.delta_f / np.sum(self.delta_f)
            mean_WA = np.sum(self.scr * w_k)
            mean_WL = np.sum((self.sf**2)*w_k) / np.sum(w_k*self.sf)
            self.mu_cr[self.k] = mean_WA
            self.mu_f[self.k] = mean_WL
            # print("delta_f", self.delta_f)
            # print("self.scr:",self.scr)
            # print("w_k:",w_k)
            self.sf = self.scr = self.delta_f = np.array([])
            self.k = (self.k+1) % self.pool_size
        self.crossover_rate = self.generate_random_nums(dist='norm')
        self.scaling_factor = self.generate_random_nums(dist='cauchy')  # 第一步，初始化cr和f
        ...
        for i in range(test_times):
            ...
            for cur_round in range(rounds):
                if cur_round % 10 == 0:
                    ...
                    func_value = SHADE.test_func(self.population.fittest_ind.genes)
...
def main():
    params = {
        ...
        "rounds": 200, "test_times": 200, "pool_size": 20,
